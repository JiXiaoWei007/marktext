...
export const listen = mtApp => {
  const { appMenu, appWindow, preference } = mtApp
  ...
  ipcMain.on('AGANI::window::drop', async (e, fileList) => {
    ...
    for (const file of fileList) {
      if (isMarkdownFileOrLink(file)) {
        openFileOrFolder(win, file, mtApp)
        break
      }
      // handle import file
  ...
  ipcMain.on('AGANI::format-link-click', (e, { data, dirname }) => {
    ...
    if (pathname) {
      const win = BrowserWindow.fromWebContents(e.sender)
      return openFileOrFolder(win, pathname, mtApp)
    }
  })
}
...
export const openFileOrFolder = (win, pathname, mtApp) => {
  const { appWindow, preference } = mtApp
  const resolvedPath = normalizeAndResolvePath(pathname)
  if (isFile(resolvedPath)) {
    const openFilesInNewWindow = preference.getItem('preference.general.openFilesInNewWindow')
...
export const openFolder = (win, mtApp) => {
  const dirList = dialog.showOpenDialog(win, {
    properties: ['openDirectory', 'createDirectory']
  })
  if (dirList && dirList[0]) {
    openFileOrFolder(win, dirList[0], mtApp)
  }
}

export const openFile = (win, mtApp) => {
  ...
  if (fileList && fileList[0]) {
    openFileOrFolder(win, fileList[0], mtApp)
  }
}

export const newFile = (mtApp) => {
  mtApp.appWindow.createWindow()
}
...
export const autoSave = (menuItem, browserWindow, mtApp) => {
  const { appWindow, preference } = mtApp
  const { checked } = menuItem
  preference.setItem('autoSave', checked)
...
export const clearRecentlyUsed = (mtApp) => {
  const { appMenu } = mtApp
